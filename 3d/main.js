...
$(function() {
    ...
    var 
        $clockDigits = $('#clock > div'),
        $counterDigits = $('#counter > div'),
        to = moment().add({days: 14}),
        ...
        tick = function() {
            ...
            $clockDigits.each(function() {

                var what = $.trim($(this).attr('class').match(/days|hours|minutes|seconds/)[0]);
                setDigitis($(this), now.get(what));

            });

            $counterDigits.each(function() {

                var what = $.trim($(this).attr('class').match(/days|hours|minutes|seconds/)[0]);
                setDigitis($(this), diff.get(what));
        ...
        setDigitis = function($el, val) {
            
            var 
                currentVal = $el.attr('data-val'),
                val = val > 9 ? "" + val : "0" + val;

            if (val != currentVal) {
              $el
                .find('div[data-position="0"] .b span').text(val).end()
                .find('div[data-position="3"] .f span').text(val).end()
                .attr('data-val', val)
                .addClass('changed')
            } else {
